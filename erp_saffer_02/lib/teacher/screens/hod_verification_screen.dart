...
class HodVerificationScreen extends StatefulWidget {
  final String department;
  final String semester;
  final String section;

  const HodVerificationScreen({
    super.key,
  ...
  @override
  State<HodVerificationScreen> createState() => _HodVerificationScreenState();
}

class _HodVerificationSkeleton extends StatelessWidget {
  const _HodVerificationSkeleton();
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: const [
        ErpSkeleton(height: 128, radius: 16),
        SizedBox(height: 16),
        ErpSkeletonListItem(),
        ErpSkeletonListItem(),
        ErpSkeletonListItem(),
...
class _HodVerificationScreenState extends State<HodVerificationScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _error = 'Failed to load HOD report computations.';
          _loading = false;
        });
      }
    }
  ...
  Future<void> _actionVerifyHod(bool approve) async {
    if (!approve && _commentController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Rejection comment/reason is required.'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              approve
                  ? 'Internal marks approved and submitted to Dean!'
                  : 'Assessments rejected back to Incharge.',
            ),
            backgroundColor:
            ...
            _loading
                ...
                : Column(
                  children: [
                    ...
                    if (currentGlobalStatus == 'CLASS_INCHARGE_VERIFIED')
                      _buildActionStrip(),
                  ],
                ),
      ),
            ...
            children: [
              Expanded(
                child: OutlinedButton(
                  ...
                  onPressed: _actioning ? null : () => _actionVerifyHod(false),
                  child: Text(
                    'Reject to Incharge',
                    style: GoogleFonts.outfit(fontWeight: FontWeight.bold),
                  ),
              ...
              Expanded(
                child: ElevatedButton(
                  ...
                  onPressed: _actioning ? null : () => _actionVerifyHod(true),
                  child: Text(
                    'Approve & Forward',
                    style: GoogleFonts.outfit(
                      fontWeight: FontWeight.bold,
